...
def main():
    if sys.argv[1] == "new":
        ...
        # Call a function "parser()" to get access and filter optional command-line arguments
        args = parser()

        # Call a function "new_activity()" which returns activity value or exit from the program(in case Bored API return error)
        add_to_db = new_activity(args)

        # Create an instance of Activity_table class
        activity_table = Activity_table()

        # Call a class method "insert_activity()" which insert new activity to the database(activity.db)
        Activity_table.insert_activity(add_to_db)

    elif sys.argv[1] == "list":
        # Call a class method "latest_activities(n)" which return the last n activities saved in the database
...
def parser():
    # Use the argparse module to deal with command-line arguments
...
def new_activity(cl_arg):
    ...
    # Use try/except. Bored API can return an error and trying accessing activity from the dictionary will raise a KeyError
    try:
        # Call a get_activity instance method to get an activity
        activity = requests.get(wrapper.get_activity())

        # Use json() method which allows use Python dictionary and get access to activity value
